...
d3.csv("/data/suicide.csv").then(function (data) {
    var suicideByCountrynDate = d3.nest()
        ...
        .rollup(function (v) {
            return {
                count: v.length,
                suicides: d3.sum(v, function (d) {
                    return d.suicides_no;
                }),
                suicidesper: d3.sum(v, function (d) {
                    return d["suicides/100k"]
                })
            };
        })
        .entries(data);
    ...
    function update() {
        ...
        if (selected.val() == "total") {
            ...
            dataset = suicideByCountrynDate[i].values.slice().sort(function (a, b) {
                return b.value.suicidesper - a.value.suicidesper;
            }).slice(0, 15).reverse()
        ...
        if (i == json_length - 1) {
            update_charts(dataset, 500, mode);
            i = 0;
        } else {
            update_charts(dataset, 500, mode);
        }
    ...
    function update_charts(data_, speed, mode) {
        ...
        var color = d3.scaleLinear().domain([1, d3.max(data_.map(function (d) {
                return d.value[mode]
            }))])
        ...
        x_bar.domain([0, d3.max(data_.map(function (d) {
            return d.value[mode]
        }))]).nice()
        g.select(".x")
            .transition()
        ...
        y_bar.domain(data_.map(function (d) {
            return d.key
        }));
        ...
        bar.enter().append("rect")
            ...
            .attr("width", d => {
                return x_bar(d.value[mode]);
            })
            .attr("fill", d => {
                return color(d.value[mode])
            });
    }
});